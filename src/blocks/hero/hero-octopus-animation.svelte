	...
	$effect(() => {
		...
		async function loop() {
			...
			while (!cancelled) {
				...
				grabbedId = targetId;
				await wait(900); // tentacle arrives at card
				if (cancelled) break;

				// Tentacle touches card → activate glow AND start pull simultaneously.
				// grabbedOffset=300 drives a CSS translateX on the card wrapper (1.3s ease-in),
				// exactly matching the octo-out tentacle retraction (also 1.3s ease-in).
				...
				await wait(1300); // both card CSS-transition and tentacle retraction finish
				if (cancelled) break;
...
-->
<div class="relative h-[520px] w-full max-w-[480px] overflow-hidden" aria-hidden="true">
	<!-- Real KanbanColumn shell + KanbanItem cards with DnD -->
	<div class="absolute" style="left: 16px; top: 80px; width: 280px;">
		<DragDropProvider
			{sensors}
					...
					{#each board.demo as task, i (task.id)}
						<div
							...
							style={task.id === grabbedId
								? `transform: translateX(${grabbedOffset}px); opacity: ${grabbedOffset > 0 ? 0 : 1}; transition: transform 1.3s ease-in, opacity 0.9s ease-in 0.4s; pointer-events: none;`
								: ''}
						>
