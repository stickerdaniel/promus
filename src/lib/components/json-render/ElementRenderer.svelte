...
{#if isVisible && Component}
	<svelte:boundary
		onerror={(error) => {
			console.error(`[json-render] Rendering error in <${resolvedElement.type}>:`, error);
		}}
	>
		<Component element={resolvedElement} bindings={elementBindings} {loading} {on} {emit}>
			{#if resolvedElement.repeat}
				<RepeatChildren element={resolvedElement} {spec} {registry} {loading} {fallback} />
			{:else if resolvedElement.children}
				{#each resolvedElement.children as childKey (childKey)}
					{#if spec.elements[childKey]}
						<Self element={spec.elements[childKey]} {spec} {registry} {loading} {fallback} />
					{:else if !loading}
						{console.warn(
