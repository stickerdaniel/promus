<script module lang="ts">
	...
	export function getActionContext(): ActionContext {
		...
		if (!ctx) {
			throw new Error('getActionContext must be called within an ActionProvider');
		}
		return ctx;
	}
	 ...
	 */
	export function getAction(name: string): CurrentValue<ActionHandler | undefined> {
...
<script lang="ts">
	...
	const execute = async (binding: CoreActionBinding): Promise<void> => {
		...
		if (resolved.action === 'validateForm') {
			...
			const statePath = (resolved.params?.statePath as string) || '/formValidation';
		...
		if (resolved.action === 'push' && resolved.params) {
			...
			if (screen) {
				const currentScreen = stateCtx.get('/currentScreen') as string | undefined;
				const navStack = (stateCtx.get('/navStack') as string[] | undefined) ?? [];
				if (currentScreen) {
		...
		const handler = registeredHandlers[resolved.action] ?? handlers[resolved.action];
