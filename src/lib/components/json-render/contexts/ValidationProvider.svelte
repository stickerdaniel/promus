<script module lang="ts">
	...
	import type { ValidationConfig, ValidationFunction, ValidationResult } from '@json-render/core';
	...
	export function getValidationContext(): ValidationContext {
		...
		if (!ctx) {
			throw new Error('getValidationContext must be called within a ValidationProvider');
		}
		return ctx;
	}
	...
	export function getOptionalValidationContext(): ValidationContext | null {
		return hasContext(VALIDATION_KEY) ? getContext<ValidationContext>(VALIDATION_KEY) : null;
	}
...
<script lang="ts">
	...
	const validate = (path: string, config: CoreValidationConfig): CoreValidationResult => {
